...
const CreateEventForm = ({ organizerId, onSuccess, onClose }) => {
  ...
  // Fetch skills grouped by category when component mounts
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        const res = await axios.get('/api/skills');
        if (res.data.success) {
          // Convert data to format for react-select grouped options
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const formattedData = {
        ...
        endDate: format(new Date(data.endDate), 'yyyy-MM-dd'),
        isRemote: data.isRemote ? 1 : 0,
        requiredSkills: data.requiredSkills, // array of skill IDs
      };
